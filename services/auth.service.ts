...
export default class AuthService extends moleculer.Service {
  ...
  async me(ctx: Context<{}, UserAuthMeta>) {
    ...
    const data: any = {
      ...
      isAgreedToTermsOfService: !!user?.isAgreedToTermsOfService,
      email: user.email,
      phone: user.phone,
      type: user.type,
    };
  ...
  async validateType(ctx: Context<{ types: EndpointType[] }, UserAuthMeta>) {
    ...
    const isAgreedToTermsOfService = !!user?.isAgreedToTermsOfService;
    ...
    if (!isAgreedToTermsOfService && types.includes(EndpointType.NON_AGREEING)) {
